...
А теперь разберем пару вопросов:

1. В чем отличие коллекции `ArrayList` и `LinkedList`?..  
**Ответ:** в основе `ArrayList` лежит массив, а в основе `LinkedList` связный список.
2. Алгоритмическая сложность получения элемента по индеску у `ArrayList` и `LinkedList`?..  
**Ответ:** `ArrayList` - O(1). `LinkedList` - O(N).
