...
По этим ссылкам можно переходить от одного элемента списка к другому. При этом нам не надо волноваться, что будет происходить утечка памяти как в случае с `JArrayList`. Если мы удаляем объект и на него нет ссылок из стэка, его удалит сборщик мусора (Garbage Collector - GC). Всё "просто".
